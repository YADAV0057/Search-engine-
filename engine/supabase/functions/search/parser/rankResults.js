...
// getRoutingForMood() + fuzzyMatch's distance fn instead of inventing new
// scoring machinery.
//
...
/**
 * textMatch sub-score, 0–1. Reuses the same Levenshtein distance fuzzyMatch.js
 * already computes for spell-correction (§5 file status table) rather than
 * building a second string-similarity function.
 * TODO: confirm the exact exported fn name/signature against the live
 * fuzzyMatch.js source before wiring this in — assuming something like
 * `levenshteinDistance(a, b)` based on the file's stated purpose in §5/§6.
 */
function textMatchScore(candidate, queryTokens) {
  ...
  for (const field of fields) {
    ...
    for (const token of queryTokens) {
      const dist = levenshteinDistance(token, fieldNorm); // from fuzzyMatch.js
      const maxLen = Math.max(token.length, fieldNorm.length);
