...
/**
 * Returns a 0–1 similarity score between two arbitrary strings, reusing
 * this file's own levenshtein() rather than duplicating edit-distance
 * logic elsewhere. 1 = identical, 0 = completely different (clamped, never
 * negative). Added 2026-07-14 specifically for rankResults.js's
 * textMatchScore(), which needs to compare a query token against one
 * candidate's title/author/character at ranking time — a different job
 * than correctWord()'s "snap this word to the nearest vocab entry"
 * lookup, so it gets its own small entry point instead of overloading
 * that one. Purely additive: does not touch correctTypos()/
 * correctTokens()/warmVocab() or any vocab-matching behavior at all.
 *
 * Note: levenshtein() short-circuits to a flat 99 when the two strings'
 * lengths differ by more than 3 (a cheap reject for correctWord()'s
 * word-vs-word use case). That means similarity() will floor to 0 for
 * e.g. a short query token compared against a long title string — expected
 * given how it's used in textMatchScore() (per-token vs a whole title
 * field), not a bug in this function.
 */
export function similarity(a, b) {
    if (!a || !b) return 0;
    const normA = String(a).toLowerCase();
