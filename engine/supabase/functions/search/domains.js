...
import { classifyQuery, rankCategories, hasStrongTitleMatch, getNegatedGenreTerms, getTagVocabEntries, getGenreVocabEntries, significantTokens } from './parser/queryClassifier.js';
...
import { getTropeSignatureFallbackForQuery } from './parser/tropeSignature.js';

// Query-time semantic search (Entry: pgvector embedding pipeline). Gated
// entirely behind filters.useSemantic -- see resolveSemanticCandidates()
// below for the full design note. Gemini is used ONLY for the embedding
// call here; Groq/Cerebras continue to own query classification and the
// emotion fallback exactly as they do everywhere else in this file.
const SEMANTIC_GEMINI_API_KEY = Deno.env.get('GEMINI_API_KEY');
// FIXED 2026-07-26: this was still pointed at 'text-embedding-004', which
// Google retired (see backfill-embeddings v8's own header note, and Entry
// 88's diagnostic finding for the full incident). Every query-time embed
// call here was therefore silently 404ing and falling back to `return
// null` -- meaning resolveSemanticCandidates() has been returning []
// unconditionally since before this feature was ever gated behind
// filters.useSemantic, regardless of that flag's value. Flipping the flag
// on (see resolveSemanticCandidates() below) would have been a complete
// no-op without this fix. Matched byte-for-byte against
// backfill-embeddings v8's already-working embedText(): model moves into
// the URL path (gemini-embedding-001, current GA model), and
// output_dimensionality: 768 is required in the body to match this
// column's existing vector(768) type (the model defaults to 3072-dim
// otherwise, which would fail on write/compare, not just embed
// differently).
const SEMANTIC_EMBED_URL =
  'https://generativelanguage.googleapis.com/v1beta/models/gemini-embedding-001:embedContent';

async function embedQueryForSemanticSearch(rawQuery) {
  if (!SEMANTIC_GEMINI_API_KEY || !rawQuery?.trim()) return null;
  try {
    const res = await fetch(`${SEMANTIC_EMBED_URL}?key=${SEMANTIC_GEMINI_API_KEY}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        content: { parts: [{ text: rawQuery }] },
        output_dimensionality: 768,
      }),
    });
    if (!res.ok) {
      console.error('[domains] Gemini query embed failed', res.status, await res.text());
      return null;
    }
    const data = await res.json();
    return data?.embedding?.values ?? null;
  } catch (err) {
    console.error('[domains] embedQueryForSemanticSearch error', err);
    return null;
  }
}

// Query-time semantic candidate pool via pgvector, gated behind
// filters.useSemantic (Entry 63/64 QA-batch A/B plan) -- when the flag is
// absent/false this is never called, so every existing query is unaffected.
//
// match_media_by_embedding() returns lexicon_entities rows (source_id, name,
// metadata, similarity), NOT adapter-shaped candidates -- shaped here into
// the same {title, genres, tags, description, popularity, chapters} shape
// every MANGA_SOURCES adapter already returns, reading the display fields
// backfill-embeddings v4 now stores in metadata alongside the embedding.
// Rows not yet re-embedded under the new metadata shape (row.metadata.title
// missing) are skipped rather than surfaced half-populated.
//
// KNOWN GAP: MangaDex-sourced rows have no real `popularity` or `chapters`
// value (see backfill-embeddings v4's header) -- these default to 0/null,
// which means filters.maxPopularity/minChapters/maxChapters in
// applyPostFetchFilters() below can exclude a semantic hit that would
// otherwise have been a great match. Flagged, not fixed here.
async function resolveSemanticCandidates(supabase, rawQuery, filters) {
  // ACTIVATED 2026-07-26: flipped from opt-in (`!filters?.useSemantic`) to
  // opt-out (`filters?.useSemantic === false`). Repo owner decided to turn
  // semantic matching on for real traffic now rather than wait for the
  // pgvector backfill to finish and/or a formal QA A/B pass first (Entry
  // 88's original recommended sequencing) -- coverage is partial (~8% of
  // 9,912 media rows embedded as of this change) but partial real
  // semantic signal for whichever titles ARE embedded is strictly better
  // than zero for every query, and this block is purely additive (see the
  // call site in runManga() below) so it can only ever add candidates,
  // never remove ones the existing waterfall would have found anyway.
  // Explicit `filters.useSemantic === false` (e.g. from the Mixer/Advanced
  // Filter path, if it's ever extended to pass filters through this
  // function directly) still fully disables this, unchanged from before.
  if (!supabase || filters?.useSemantic === false) return [];
  const queryEmbedding = await embedQueryForSemanticSearch(rawQuery);
  if (!queryEmbedding) return [];

  try {
    const { data, error } = await supabase.rpc('match_media_by_embedding', {
      query_embedding: queryEmbedding,
      match_count: 50,
    });
    if (error) {
      console.error('[domains] match_media_by_embedding failed', error);
      return [];
    }

    return (data || [])
      .filter((row) => row.metadata?.title)
      .map((row) => ({
        id: row.source_id,
        title: { romaji: row.metadata.title, english: row.metadata.title },
        genres: row.metadata.genres || [],
        tags: (row.metadata.genres || []).map((name) => ({ name })),
        description: row.metadata.description || '',
        popularity: row.metadata.popularity || 0,
        chapters: row.metadata.chapters ?? null,
        semanticScore: row.similarity,
      }));
  } catch (err) {
    console.error('[domains] resolveSemanticCandidates error', err);
    return [];
  }
}
...
async function computeMoodSignal(supabase, rawQuery, groqApiKey, cerebrasApiKey) {
  ...
  try {
    ...
    if (!hasPositive && !hasNegated) {
      ...
      if (idiomOnly) return { ...idiomOnly, cleanTokens, claimed, negated };
      const intentFallback = await getEmotionalIntentFallback(rawQuery, tokens, groqApiKey, cerebrasApiKey, supabase);
      return intentFallback ? { ...intentFallback, cleanTokens, claimed, negated } : null;
    }
    ...
    return {
      ...
      matchedTerms,
      // Added for parser/tropeSignature.js's query-time fallback (Entry
      // 71/72) -- reuses the same tokens/coverage-mask idiomFallback.js
      // already computed a few lines above, rather than re-tokenizing or
      // re-running analyzeQueryMood() a second time in runManga(). NOTE:
      // the "both idiom and emotional-intent fallback found nothing"
      // branch above (a few lines up) still returns null in that one
      // case, same as before this change -- so the trope fallback won't
      // fire on a query where literally every other signal source also
      // came up empty. Narrow edge case, flagged rather than fixed here
      // (fixing it means changing what `mood: null` vs `mood: {}` means
      // to every other `mood ?` check in this file -- out of scope for
      // this addition).
      cleanTokens,
      claimed,
      negated,
...
async function runManga({ query, filters, supabase }) {
  ...
  const routing = mood ? getRoutingForMood(mood.aggregate) : { boostGenres: [], excludeGenres: [], emotionGenreMap: [] };

  // Entry 66. Real AniList tag vocabulary (~420 names), for rankResults.js's
  // Tier-1 exact-tag scoring. Reuses getTagVocabEntries() -- already
  // imported, already used by resolveMoodTagCandidates() below, and
  // already warm-cached by queryClassifier.js -- so this is a cache read,
  // not a new DB round trip. Built unconditionally (cheap) rather than
  // gated on `mood` so it's ready for both rankResults() call sites below.
  const tagVocabEntries = await getTagVocabEntries(supabase);
  const tagVocab = new Set(tagVocabEntries.map((e) => (e.name || '').toLowerCase()));

  // Entry 71/72. Real AniList genre vocabulary (19 rows as of Entry 37/41),
  // for tropeSignature.js's LLM prompt -- same warm-cache reuse as
  // tagVocabEntries immediately above, see queryClassifier.js's
  // getGenreVocabEntries() for why this is a cache read, not a new query.
  const genreVocabEntries = await getGenreVocabEntries(supabase);

  // Entry 71/72: query-time trope-signature fallback. Fires on whatever
  // candidate phrase(s) tropeSignature.js's own span extraction finds
  // uncovered in THIS user's live query -- "candidate phrase comes from
  // user queries as they search" (repo owner, Entry 72) rather than
  // waiting on a static list or a separate classification lane (Entry 67's
  // fuller "fourth query-classification lane" is still the more correct
  // long-term home for this, not built here). Guarded on `mood` existing
  // since it reuses the same cleanTokens/claimed/negated coverage mask
  // idiomFallback.js already computed inside computeMoodSignal() above --
  // see that function's return statement for the one narrow case
  // (everything else also found nothing) where this won't fire.
  const tropeSignal = mood
    ? await getTropeSignatureFallbackForQuery(
        mood.cleanTokens,
        mood.claimed,
        mood.negated,
        tagVocabEntries,
  ...
  // Entry 71/72: fold a confidently-classified trope's real AniList genre
  // names into the fetch plan, same guard shape as the genreOnlyTerms
  // block immediately above (skip when an explicit filter-panel selection
  // or a "like X" reference-title resolution already decided the genre
  // scope). tropeSignal.genreWeights keys are already correctly-cased
  // AniList genre strings (sourced from getGenreVocabEntries(), the real
  // harvested vocabulary) -- no Entry-52-style case translation needed,
  // same reasoning as genreOnlyTerms above.
  //
  // NOT done here (flagged, not built): tropeSignal.tagWeights is computed
  // and available but not yet threaded into rankResults()'s tagVocab-based
  // Tier-1 scoring (Entry 66's emotionGenreMap/tagVocab path) -- that would
  // give trope matches real per-candidate tag-overlap ranking instead of
  // just a genre-level fetch filter. Worth a follow-up once this fires
  // live enough to be worth tuning; genre-level filtering alone is still a
  // real improvement over the previous "trope phrase gets zero signal"
  // baseline.
  if (
    !hasExplicitGenreFilter &&
    !referenceResolution.plan &&
    tropeSignal &&
    Object.keys(tropeSignal.genreWeights || {}).length > 0
  ) {
    const tropeGenreNames = Object.keys(tropeSignal.genreWeights);
    plan.primaryGenres = [...new Set([...(plan.primaryGenres || []), ...tropeGenreNames])];
  }
  ...
  // Query-time semantic candidate pool (pgvector), gated behind
  // filters.useSemantic -- see resolveSemanticCandidates()'s header above.
  // Skipped in fan-out mode for the same reason moodTagResolution is:
  // Advanced Filter's filters-only request shape has no free text to embed.
  const semanticResolution = fanOut
    ? []
    : await resolveSemanticCandidates(supabase, query || '', filters);
  ...
  if (fanOut) {
    ...
    resultsFO = rankResults(resultsFO, {
      ...
      emotionGenreMap: routing.emotionGenreMap,
      tagVocab,
      moodMatchedTerms,
    ...
    return {
      ...
      referenceTitle: null,
      moodTags: null,
      // Entry 71/72. Unlike referenceTitle above, tropeSignal IS computed
      // for fan-out requests (it only depends on mood's token/coverage
      // mask, not on the free-text-sentence assumption referenceTitle.js
      // makes) -- surfaced here for real, not stubbed to null.
      tropeSignature: tropeSignal,
    };
  }
  ...
  // Semantic hits seeded third -- after reference-title recommendations
  // (highest precision) and mood-tag matches, before the generic waterfall.
  // Same additive shape as the two blocks above: dedup by title, stop once
  // `limit` is hit, contributes its own named source. Only ever non-empty
  // when filters.useSemantic was actually requested (resolveSemanticCandidates
  // returns [] otherwise), so this block is a pure no-op for every existing
  // caller that doesn't opt in.
  if (semanticResolution.length > 0) {
    const filtered = applyPostFetchFilters(semanticResolution, filters) || [];
    let contributed = false;
    for (const item of filtered) {
      if (accumulated.length >= limit) break;
      const key = normalizeTitleKey(item);
    ...
    if (contributed) contributingSources.push('semantic-embedding');
  }
  ...
  if (accumulated.length > 0) {
    ...
    results = rankResults(results, {
      ...
      conjunctiveClusters,
      hasStrongTitleMatch: classification.hasStrongTitleMatch,
      // ACTIVATED 2026-07-26 -- matches resolveSemanticCandidates()'s new
      // opt-out default above. A candidate only ever has a real
      // semanticScore when resolveSemanticCandidates() actually ran and
      // found it (i.e. useSemantic wasn't explicitly false), so this stays
      // consistent with whatever the fetch side actually did.
      semanticEnabled: filters?.useSemantic !== false,
    });
