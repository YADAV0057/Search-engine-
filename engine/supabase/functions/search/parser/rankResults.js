...
//
// ADDED (semantic search integration): a pgvector cosine-similarity term,
// semanticScore, carved out of the intent budget the exact same way quality
// is above — a flat MAX_SEMANTIC_WEIGHT share, but only when the caller
// explicitly passes semanticEnabled: true (domains.js only does this when
// filters.useSemantic was requested — see resolveSemanticCandidates() in
// domains.js). semanticWeight is 0 for every query that doesn't opt in, so
// this is a strict extension: intentScore === the pre-existing formula for
// every query that isn't part of the Entry 63/64 QA-batch A/B test.
...
// Ceiling on how much of the intent budget semantic similarity can claim.
// Unlike qualityWeight, this isn't scaled by a per-query intensity signal --
// it's a flat on/off carve-out, since "was useSemantic requested" is binary,
// not a matter of degree. 0 whenever semanticEnabled is false (the default
// for every query not explicitly opted into the Entry 63/64 QA-batch A/B),
// so existing ranking behavior is completely unaffected unless a caller
// explicitly asks for this.
const MAX_SEMANTIC_WEIGHT = 0.30;
 ...
 *
 * semanticEnabled: boolean, defaults to false. Only true when domains.js's
 * runManga() actually ran resolveSemanticCandidates() (filters.useSemantic
 * was requested) — see this file's top-of-file note. When false, every
 * candidate's semanticMatch is forced to 0 and semanticWeight is 0, so
 * intentScore is byte-identical to this file's pre-semantic behavior.
 */
function rankResults(results, { classifierRanked, moodAggregate, queryTokens, queryGenreTerms, filterGenres, boostGenres, emotionGenreMap, tagVocab, moodMatchedTerms, acclaimIntensity = 0, conjunctiveClusters = null, hasStrongTitleMatch = false, semanticEnabled = false }) {
  ...
  const scored = partial.map(({ candidate, textMatch, genreMatch, emotionMatch, popularity, quality }) => {
    ...
    // semanticScore comes from resolveSemanticCandidates() in domains.js
    // (the pgvector similarity, 0-1) -- present only on candidates seeded
    // from that path; every candidate from the four MANGA_SOURCES adapters
    // simply has no semanticScore field, so `?? 0` there is correct, not a
    // failure case.
    const semanticMatch = semanticEnabled ? (candidate.semanticScore ?? 0) : 0;

    // Entry 35/40 + semantic addition. Quality and semantic each only ever
    // displace their own carved-out share of the existing intent budget —
    // qualityWeight is 0 without acclaim intent, semanticWeight is 0
    // without useSemantic, so intentScore === baseIntentScore whenever
    // neither is active, unchanged from before this patch.
    const intentScore = remainingWeight * baseIntentScore + qualityWeight * quality + semanticWeight * semanticMatch;
    ...
    return {
      ...
      _rankDebug: {
        ...
        quality, qualityWeight, semanticMatch, semanticWeight, weights, saturated, conjunctionMatch, finalScore
      },
      finalScore
    };
  });
